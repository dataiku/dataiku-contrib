...
class USCensusConnector(Connector):
    ...
    def generate_rows(self, dataset_schema=None, dataset_partitioning=None,
        ...
        logger.info('----------------------------------------')
        if s_found >0:
            logger.info('States expected to be processed if enough records for feature selection:')
            logger.info(state_list)
            logger.info('States rejected:')
            if s_rejected < 60:
                logger.info(state_list_rejected)
            else:
                logger.info('...too many elements rejected for displaying it in the log...')
            ...
            for state in state_list:
                ...
                logger.info('https://www.census.gov/geo/maps-data/data/tallies/tractblock.html')
                logger.info('----------------------------------------------')
            ...
            if self.P_DELETE_US_CENSUS_SOURCES is True:
                ...
                logger.info('6/6 Removing US Census temp data from: %s' % (path_datadir_tmp + FOLDER_NAME))
                cmd = "rm -rf %s" % (path_datadir_tmp + FOLDER_NAME)
                os.system(cmd)

            else:
                logger.info('6/6 Keeping US Census data sources in: %s' % (path_datadir_tmp + FOLDER_NAME))
            ...
            logger.info('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            logger.info('US Census CANNOT be built, no states available in your dataset...')
            logger.info('Check the following settings :')
            logger.info('-> are the states in the right format regarding the plugin set by the user ?')
