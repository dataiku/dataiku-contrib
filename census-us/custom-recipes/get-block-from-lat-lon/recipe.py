...
with output_dataset.get_writer() as writer:
    for df in dataiku.Dataset(input_name).iter_dataframes(chunksize= P_BATCH_SIZE_UNIT ):
        ...
        for v in dfu.to_dict('records'):
            ...
            logging.info('%s - processing: (%s,%s)' % (n_record,lat, lon))
            ...
            if call.status_code == 200:
                ...
                try:
                    ...
                    logging.info('Unable to find these coordinates in the US Census API: lat:%s , lon:%s' % (lat,lon))

            else:
                logging.info('Failed. API status: %s' % (call.status_code))
                logging.info('The plugin will write the output dataset where the process stopped. You should probably consider filtering your input dataset where the plugin stopped and select the append mode for the input/output panel.')
                sys.exit(1) 
