    ...
    for chunk_idx, df in enumerate(input_dataset.iter_dataframes(chunksize=CHUNK_SIZE)):
        ...
        if output_sentiment:
            if sentiment_scale == 'binary':
                ...
                df[new_cols[1]] = [ "highly negative" if p==1 else "negative" if p==2 else "neutral" if p==3 else "positive" if p==4 else "highly positive" if p==5 else "" for p in predicted_scores]
