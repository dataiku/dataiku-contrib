...
OUTPUT_COLUMN   = get_recipe_config().get('output-column', 'score')
...
endpoint = "https://{}.api.cognitive.microsoft.com/text/analytics/v2.0".format(AZURE_LOCATION)
service  = "/sentiment"
...
for group in grouper(BATCH_SIZE, iterator):
    ...
    for i, record in enumerate(group):
        o = dict(record)
        try:
            o[OUTPUT_COLUMN] = json.dumps( r.json()["documents"][i]["score"] )
        except:
