...
truncate_date <- function(date, granularity) {
...
aggregate_na <- function(x, strategy){
...
convert_df_to_ts <- function(df, time_column, series_column, granularity) {
...
clean_ts <- function(ts, missing_values, missing_impute_with, missing_impute_constant, 
    ...
    if (missing_values == 'interpolate') {
        ...
        missing_imputation <- case_when(
            missing_impute_with == 'median' ~ median(ts, na.rm=TRUE),
            missing_impute_with == 'average' ~ mean(ts, na.rm=TRUE),
            missing_impute_with == 'constant' ~ missing_impute_constant
        )   
        msts[which(is.na(ts))] <- missing_imputation
    }
    
    outliers_detected <- tsoutliers(ts)
    if(outliers == 'interpolate'){
        ts[outliers_detected$index] <- outliers_detected$replacements
    }
    else if (outliers == 'impute') {
        outliers_imputation <- case_when(
            outliers_impute_with == 'median' ~ median(ts, na.rm=TRUE),
            outliers_impute_with == 'average' ~ mean(ts, na.rm=TRUE),
            outliers_impute_with == 'constant' ~ outliers_impute_constant
        )   
        ts[outliers_detected$index] <- outliers_imputation
    }
    return(ts)
}

clean_df <- function(df, time_column, series_columns, granularity, 
    ...
    for(c in series_columns){
        ts <- convert_df_to_ts(df, time_column, c, granularity)
        df_output[[c]] <- as.numeric(
            clean_ts(ts, missing_values, missing_impute_with, missing_impute_constant, 
                outliers, outliers_impute_with, outliers_impute_constant
            )
        )
    }   
