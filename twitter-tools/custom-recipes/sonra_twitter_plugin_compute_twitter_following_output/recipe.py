...
# Delay in seconds
FOLLOWING_INTERVAL=int(get_recipe_config()['following_interval'])
...
for record in ud.iter_rows():
    print "Follow user: "+record[INPUT_COLUMN]
    o = {}
    o["screen_name"] = record[INPUT_COLUMN]
    # follow user
    try:
        response = client.api.friendships.create.post(screen_name=record[INPUT_COLUMN])
